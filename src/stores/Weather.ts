...
export class Weather extends Store implements IWeatherStore {
  ...
  addFavoriteCity = (city: FavoriteCity) => {
    ...
    this.appStore.setAlert({ content: `"${genKey(city, " - ")}" added to favorite` })
  }
  ...
  removeFavoriteCity = (city: FavoriteCity) => {
    ...
    this.commitFavoriteCities()
    this.appStore.setAlert({ content: `"${genKey(city, " - ")}" removed from favorite` })
  }
  ...
  commitCitiesWeather = () => {
    ...
    const citiesWeather = parse(localStorage.getItem("citiesWeather") || noData) 

    // cities weather creation order saved in the localStorage.
    const citiesWeatherCreationOrder = parse(localStorage.getItem("citiesWeatherCreationOrder") || noData)
    let creationOrders: Array<[string, number]> = Object.entries(citiesWeatherCreationOrder)
    ...
    localStorage.setItem("citiesWeatherCreationOrder", stringify(citiesWeatherCreationOrder))
  }
  ...
  getCachedCityWeather = (key: string) => {
    ...
    return !isEmptyValue(citiesWeather)
      ? citiesWeather[key]
      : {}
  }
